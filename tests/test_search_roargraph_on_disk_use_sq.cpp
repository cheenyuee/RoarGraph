...
#include "index_bipartite_disk.h"
...
int main(int argc, char **argv) {
    ...
    std::string disk_index_save_file;
    ...
    try {
        ...
        desc.add_options()("disk_index_save_path",
                            po::value<std::string>(&disk_index_save_file)->required(),
                            "Path prefix for saving disk index file components");
    ...
    // search on disk
    std::ofstream disk_index_path_ofs;
    if (!disk_index_save_file.empty()) {
        disk_index_path_ofs.open(disk_index_save_file.c_str());
    } else {
    ...
    index.PrepareDiskSearch(disk_index_save_file, num_threads);
